...
__device__ __half get_score_ab(const __half* body_part_a,
  ...
  const auto vector_a_to_b_max =
      __hmax(__habs(vector_a_to_bx), __habs(vector_a_to_by));
  const auto number_points_in_line =
      std::max(5, std::min(25, __half2int_rn(hsqrt(static_cast<__half>(5) *
                                                   vector_a_to_b_max))));
  const auto vector_norm =
      hsqrt(vector_a_to_bx * vector_a_to_bx + vector_a_to_by * vector_a_to_by);

  // if the peaks_ptr are coincident. don'__half connect them.
  if (vector_norm > static_cast<__half>(1e-6)) {
    ...
    auto sum = static_cast<__half>(0.f);
    auto count = 0;
    const auto number_points_in_line_half =
        static_cast<__half>(number_points_in_line);
    ...
    for (auto lm = 0; lm < number_points_in_line; lm++) {
      const auto m_x = std::min(
          heatmap_width - 1, __half2int_rn(s_x + static_cast<__half>(lm) *
                                                     vector_a_to_bx_in_line));
      const auto m_y = std::min(
          heatmap_height - 1, __half2int_rn(s_y + static_cast<__half>(lm) *
                                                      vector_a_to_by_in_line));
    ...
    // return paf score
    const auto count_half = static_cast<__half>(count);
    if (count_half / number_points_in_line_half > inter_min_above_threshold)
      ...
      //        missing
      const auto l2_dist = hsqrt(vector_a_to_bx * vector_a_to_bx +
                                 vector_a_to_by * vector_a_to_by);
      const auto threshold =
          hsqrt(heatmap_width * heatmap_height) /
          static_cast<__half>(150);  // 3.3 for 368x656, 6.6 for 2x resolution
      if (l2_dist < threshold)
        return default_nms_threshold +
               static_cast<__half>(1e-6);  // without 1e-6 will not work
...
__global__ void paf_score_kernel(
  ...
  if (peak_a < max_peaks && peak_b < max_peaks) {
    ...
    const __half number_peaks_a = peaks_ptr[3 * part_a * (max_peaks + 1)];
    const __half number_peaks_b = peaks_ptr[3 * part_b * (max_peaks + 1)];

    const auto output_index =
        (pair_index * max_peaks + peak_a) * max_peaks + peak_b;
    if (static_cast<__half>(peak_a) < number_peaks_a &&
        static_cast<__half>(peak_b) < number_peaks_b) {
      const auto map_idx_x = map_idx_ptr[base_index];
      const auto map_idx_y = map_idx_ptr[base_index + 1];

      const __half* const body_part_a =
          peaks_ptr + (3 * (part_a * (max_peaks + 1) + peak_a + 1));
      const __half* const body_part_b =
          peaks_ptr + (3 * (part_b * (max_peaks + 1) + peak_b + 1));
      const __half* const map_x =
          heat_map_ptr + map_idx_x * heatmap_width * heatmap_height;
      const __half* const map_y =
