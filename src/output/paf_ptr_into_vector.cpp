...
void paf_ptr_into_vector(int* sorted_ptr, float* total_score_ptr,
  ...
  for (auto pair_index = 0u; pair_index < number_body_part_pairs;
    ...
    const auto number_peaks_a =
        static_cast<int>(std::round(candidate_a_ptr[0]));
    const auto number_peaks_b =
        static_cast<int>(std::round(candidate_b_ptr[0]));
    ...
    for (auto index_a = 0; index_a < number_peaks_a; index_a++) {
      ...
      for (auto index_b = 0; index_b < number_peaks_b; index_b++) {
        ...
        if (score_ab > 1e-6) {
          ...
          const auto total_score = score_ab + 0.1f * peaks_ptr[index_score_a] +
